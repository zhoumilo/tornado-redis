...
# ZSET command tests have been temporary disabled 
'''
    @async_test
    @gen.engine
    def test_zsets(self):
        ...
        self.assertEqual(res, 1)
        res = yield gen.Task(self.client.zrangebyscore, 'foo', '-inf', '+inf', None, None, False)
        self.assertEqual(res, ['a', 'b', 'c'])
        res = yield gen.Task(self.client.zrangebyscore, 'foo', '2.1', '+inf', None, None, True)
        self.assertEqual(res, [('b', 3.0), ('c', 3.5)])
        res = yield gen.Task(self.client.zrangebyscore, 'foo', '-inf', '3.0', 0, 1, False)
        self.assertEqual(res, ['a'])
        res = yield gen.Task(self.client.zrangebyscore, 'foo', '-inf', '+inf', 1, 2, False)
        ...
        # max, no weight
        res = yield gen.Task(self.client.zinterstore, 'z', ['a', 'b', 'c'], aggregate='MAX')
        self.assertEqual(res, 2)
        res = yield gen.Task(self.client.zrange, 'z', 0, -1, with_scores=True)
        self.assertEqual(res, [('a3', 5), ('a1', 6)])
        # with weight
        res = yield gen.Task(self.client.zinterstore, 'z', {'a': 1, 'b': 2, 'c': 3})
        self.assertEqual(res, 2)
        res = yield gen.Task(self.client.zrange, 'z', 0, -1, with_scores=True)
        ...
        self.assertEqual(res, 4)
        res = yield gen.Task(self.client.zrange, 'z', 0, -1, with_scores=True)
        self.assertEqual(dict(res), dict([('a2', 1),
                                        ('a3', 3),
                                        ('a5', 4),
                                        ('a4', 7),
                                        ('a1', 9)]))
        # max, no weight
        res = yield gen.Task(self.client.zunionstore, 'z', ['a', 'b', 'c'], aggregate='MAX')
        self.assertEqual(res, 5)
        res = yield gen.Task(self.client.zrange, 'z', 0, -1, with_scores=True)
        self.assertEqual(res, [('a2', 1),
                               ('a3', 2),
                               ('a5', 4),
                               ('a4', 5),
                               ('a1', 6)])
        # with weight
        res = yield gen.Task(self.client.zunionstore, 'z', {'a': 1, 'b': 2, 'c': 3})
        self.assertEqual(res, 5)
        res = yield gen.Task(self.client.zrange, 'z', 0, -1, with_scores=True)
        self.assertEqual(res, [('a2', 1),
                               ('a3', 5),
...
'''
    ...
    def test_long_zset(self):
        ...
        self.assertEqual(res, 1)
        yield gen.Task(self.client.zrange, 'foobar', 0, NUM, with_scores=False, callbacks=[self.expect(long_list),  self.finish])
        self.stop()
    ...
    def test_sort(self):
        def make_list(key, items, expect_value=True):
            yield gen.Task(self.client.delete, key, callbacks=self.expect(expect_value))
            for i in items:
                yield gen.Task(self.client.rpush, key, i)
        yield gen.Task(self.client.sort, 'a', callbacks=self.expect([]))
        make_list('a', '3214', False)
        yield gen.Task(self.client.sort, 'a', callbacks=self.expect(['1', '2', '3', '4']))
        yield gen.Task(self.client.sort, 'a', start=1, num=2, callbacks=self.expect(['2', '3']))

        yield gen.Task(self.client.set, 'score:1', 8, callbacks=self.expect(True))
        yield gen.Task(self.client.set, 'score:2', 3, callbacks=self.expect(True))
        yield gen.Task(self.client.set, 'score:3', 5, callbacks=self.expect(True))
        make_list('a_values', '123')
        yield gen.Task(self.client.sort, 'a_values', by='score:*', callbacks=self.expect(['2', '3', '1']))

        yield gen.Task(self.client.set, 'user:1', 'u1', callbacks=self.expect(True))
        yield gen.Task(self.client.set, 'user:2', 'u2', callbacks=self.expect(True))
        yield gen.Task(self.client.set, 'user:3', 'u3', callbacks=self.expect(True))

        make_list('a', '231')
        yield gen.Task(self.client.sort, 'a', get='user:*', callbacks=self.expect(['u1', 'u2', 'u3']))

        make_list('a', '231')
        yield gen.Task(self.client.sort, 'a', desc=True, callbacks=self.expect(['3', '2', '1']))

        make_list('a', 'ecdba')
        yield gen.Task(self.client.sort, 'a', alpha=True, callbacks=self.expect(['a', 'b', 'c', 'd', 'e']))

        make_list('a', '231')
        yield gen.Task(self.client.sort, 'a', store='sorted_values', callbacks=self.expect(3))
        yield gen.Task(self.client.lrange, 'a', 0, -1, callbacks=self.expect(['1', '2', '3']))
        ...
        yield gen.Task(self.client.sort, 'gods',
                         start=2,
                         num=4,
                         by='user:*:username',
        ...
        yield gen.Task(self.client.lrange, 'sorted', 0, -1, callback=self.expect(['vodka',
                                                                    'milk',
                                                                    'gin',
                                                                    'apple juice',
                                                                    ]))
